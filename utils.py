...
class WebotsVersion:
    def __init__(self, version):
        ...
        # Parse
        parts = re.findall(
            r"R(\d*)([a-d]{1})(\s((revision)|(rev))\s(\d){1}){0,1}", version
        )
...
def get_host_ip():
    ...
    try:
        output = subprocess.run(
            ["ip", "route"], check=True, stdout=subprocess.PIPE, universal_newlines=True
        )
...
def get_webots_home(show_warning=False):
    ...
    for variable in environment_variables:
        if (
            ...
            and os.path.isdir(os.environ[variable])
            and WebotsVersion.from_path(os.environ[variable])
        ):
            os.environ["WEBOTS_HOME"] = os.environ[variable]
            return os.environ[variable]
        elif variable in os.environ and (
            not os.path.isdir(os.environ[variable])
            or WebotsVersion.from_path(os.environ[variable]) is None
        ):
            print(
                f"WARNING: Webots directory `{os.environ[variable]}` specified in `{variable}` is not a valid Webots "
    ...
    paths.append(
        os.path.join(
            str(Path.home()), ".ros", "webots" + minimum_version.short(), "webots"
        )
    )

    # Check if default pathes contain target version of Webots
    for path in paths:
        if os.path.isdir(path) and version_min(
            WebotsVersion.from_path(path), minimum_version
        ):
            os.environ["WEBOTS_HOME"] = path
            if show_warning:
                print(
                    "WARNING: No valid Webots directory specified in `ROS2_WEBOTS_HOME` and `WEBOTS_HOME`, fallback to "
...
def __install_webots(installation_directory):
    ...
    if is_wsl():
        ...
        installation_path = os.path.abspath(
            os.path.join(installation_directory, "webots")
        )
    ...
    if not os.path.exists(archive_path):
        ...
        urllib.request.urlretrieve(
            url + archive_name, archive_path, reporthook=on_download_progress_changed
        )
        print("")
    else:
    ...
    if is_wsl():
        ...
        installation_subdirectory = os.path.join(
            installation_directory, "webots" + minimum_version.short()
        )
        print("Extracting...")
...
def handle_webots_installation():
    ...
    webots_release_url = (
        f"https://github.com/cyberbotics/webots/releases/tag/{minimum_version.short()}"
    )
